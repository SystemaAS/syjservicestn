...
import no.systema.jservices.tvinn.sad.digitoll.model.dao.services.SadmolhffDaoServices;
...
@Controller
public class JsonResponseOutputterController_SADMOLHFF {
	private static Logger logger = LoggerFactory.getLogger(JsonResponseOutputterController_SADMOLHFF.class.getName());
	/**
	 * File: 	SADMOLHFF
	 * Member: 	SADMOLHFF Digitoll - SELECT SPECIFIC Error
	 * 
	 * @Example SELECT: http://gw.systema.no:8080/syjservicestn/syjsSADMOLHFF.do?user=OSCAR&lnrt=...
	 *
	 * 
	 *
	*/
	@RequestMapping(value="syjsSADMOLHFF.do", method={RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public String syjsRList( HttpSession session, HttpServletRequest request) {
		...
		try{
			logger.warn("Inside syjsSADMOLHFF");
			...
			if(userName!=null && !"".equals(userName)){
				...
				SadmolhffDao dao = new SadmolhffDao();
				...
				logger.warn("find");
				list = this.sadmolhffDaoServices.find(dao, dbErrorStackTrace);
	...
	@RequestMapping(value="syjsSADMOLHFF_U.do", method={RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public String syjsR_U( HttpSession session, HttpServletRequest request) {
		...
		try{
			logger.warn("Inside syjsSADMOLHFF_U.do");
			//TEST-->logger.info("Servlet root:" + AppConstants.VERSION_SYJSERVICES);
			String user = request.getParameter("user");
            ...
            logger.warn("ehdkh:" + dao.getEhdkh().toString());
            logger.warn("ehlnrt:" + dao.getEhlnrt());
			...
			//Start processing now
			if(userName!=null && !"".equals(userName)){
				if(StringUtils.isNotEmpty(dao.getUuid()) && StringUtils.isNotEmpty(dao.getEhdkh()) && StringUtils.isNotEmpty(dao.getAvsid()) 
						&& StringUtils.isNotEmpty(dao.getMotid()) ) {
					int dmlRetval = 0;
					
					logger.warn("Before INSERT ...");
					List<SadmolhffDao> list = new ArrayList<SadmolhffDao>();
					
					//do ADD
					if("A".equals(mode)){
						dmlRetval = this.sadmolhffDaoServices.insert(dao, dbErrorStackTrace);
					}else{
						//write JSON error output
	...
	@Autowired
	private SadmolhffDaoServices sadmolhffDaoServices;
