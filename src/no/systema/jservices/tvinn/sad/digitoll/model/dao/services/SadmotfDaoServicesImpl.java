...
public class SadmotfDaoServicesImpl implements SadmotfDaoServices {
	...
	public List find(Object obj,StringBuffer errorStackTrace){
		...
		try{
			...
			if(dao.getEtpro()>0 || dao.getEtpro()<0){ sql.append(" and etpro = ? "); params.add(dao.getEtpro()); }
	...
	public int update(Object daoObj, StringBuffer errorStackTrace){
		...
		try{
			...
			sql.append(" etemr = ? , etemrt = ? , etemrx = ? , etemrtx = ? , etkmrk = ? , etktm = ? , etktyp = ?, etklk = ?, etcref = ?, etktkd = ?, etsjaf = ?, etems = ?, etemst = ?, ");
			sql.append(" etknt = ? , etrgt = ? , etnat = ?, etna2t = ?, etad1t = ?, etnrt = ?, etpnt = ?, etpst = ?, etlkt = ?, etpbt = ?, ");
			sql.append(" etemt = ? , etemtt = ? , etemtx = ? , etemttx = ? , etdkm = ? , etdkmt = ? , ettsd = ? ");
			...
			dao.getEtemr(), dao.getEtemrt(), dao.getEtemrx(), dao.getEtemrtx(), dao.getEtkmrk(), dao.getEtktm(), dao.getEtktyp(), dao.getEtklk(), dao.getEtcref(), dao.getEtktkd(), dao.getEtsjaf(), dao.getEtems(), dao.getEtemst(),
			...
			dao.getEtemt(), dao.getEtemtt(), dao.getEtemtx(), dao.getEtemttx(), dao.getEtdkm(), dao.getEtdkmt(), dao.getEttsd(), 
			//id's
			dao.getEtlnrt(),
			} );
	...
	public int resetMrn(Object daoObj, StringBuffer errorStackTrace){
		...
		try{
			...
			StringBuffer sql = new StringBuffer();
			//DEBUG --> logger.info("mydebug");
			sql.append(" UPDATE " + this.TABLE_NAME + " set etmid = ? ");
			//id's
			sql.append(" WHERE etmid_own = ? ");
			
			//params
			retval = this.jdbcTemplate.update( sql.toString(), new Object[] { dao.getEtmid_own(), dao.getEtmid_own()} );
	...
	public int resetMrnManually(Object daoObj, StringBuffer errorStackTrace){
		...
		try{
			...
			StringBuffer sql = new StringBuffer();
			//DEBUG --> logger.info("mydebug");
			sql.append(" UPDATE " + this.TABLE_NAME + " set etmid = ?, etmid_own = ? ");
			//id's
			sql.append(" WHERE etlnrt = ? ");
			
			//params
			retval = this.jdbcTemplate.update( sql.toString(), new Object[] { dao.getEtmid_own(), dao.getEtmid_own(), dao.getEtlnrt() } );
