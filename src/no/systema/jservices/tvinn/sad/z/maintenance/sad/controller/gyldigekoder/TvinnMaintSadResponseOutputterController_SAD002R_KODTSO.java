...
public class TvinnMaintSadResponseOutputterController_SAD002R_KODTSO {
	private static Logger logger = LoggerFactory.getLogger(TvinnMaintSadResponseOutputterController_SAD002R_KODTSO.class.getName());
	 ...
	 * 
	 * @return
	 * @Example SELECT *: http://gw.systema.no:8080/syjservicestn/syjsSAD002_KODTSOR.do?user=OSCAR
	 * @Example SELECT specific: http://gw.systema.no:8080/syjservicestn/syjsSAD002_KODTSOR.do?user=OSCAR&ksefyl=01
	 * 
	 */
	...
	public String syjsSAD002_KODtsoR( HttpSession session, HttpServletRequest request) {
		...
		try{
			logger.info("Inside syjsSAD002_KODTSOR.do");
			...
			if(userName!=null && !"".equals(userName)){
	            ...
	            if( (dao.getKsokd()!=null && !"".equals(dao.getKsokd())) ){
	            	...
	            	list = this.kodtsoDaoServices.findById(dao.getKsokd(), dbErrorStackTrace);
	            }else{
	            	logger.info("getList (all)");
					list = this.kodtsoDaoServices.getList(dbErrorStackTrace);
	...
	public String syjsSAD002R_U( HttpSession session, HttpServletRequest request) {
		...
		try{
			logger.info("Inside syjsSAD002_KODTSOR_U.do");
			//TEST-->logger.info("Servlet root:" + AppConstants.VERSION_SYJSERVICES);
			String user = request.getParameter("user");
            ...
            binder.bind(request);
            //rules
            SAD002_KODTSOR_U rulerLord = new SAD002_KODTSOR_U();
			//Start processing now
			if(userName!=null && !"".equals(userName)){
				int dmlRetval = 0;
				if("D".equals(mode)){
					if(rulerLord.isValidInputForDelete(dao, userName, mode)){
						dmlRetval = this.kodtsoDaoServices.delete(dao, dbErrorStackTrace);
					}else{
						//write JSON error output
				  ...
				  if(rulerLord.isValidInput(dao, userName, mode)){
						List<KodtsoDao> list = new ArrayList<KodtsoDao>();
						...
						if("A".equals(mode)){
							...
							if(list!=null && list.size()>0){
								...
								dmlRetval = this.kodtsoDaoServices.insert(dao, dbErrorStackTrace);
							}
						}else if("U".equals(mode)){
							logger.info("Before UPDATE ...");
							dmlRetval = this.kodtsoDaoServices.update(dao, dbErrorStackTrace);
						}
	...
	//WIRED SERVICES
	//----------------
	@Qualifier ("kodtsoDaoServices")
	private KodtsoDaoServices kodtsoDaoServices;
	@Autowired
	@Required
	public void setKodtsoDaoServices (KodtsoDaoServices value){ this.kodtsoDaoServices = value; }
	public KodtsoDaoServices getKodtsoDaoServices(){ return this.kodtsoDaoServices; }
