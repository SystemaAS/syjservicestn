...
import no.systema.jservices.tvinn.sad.sadimport.model.dao.services.SadhDaoServices;
...
@Controller
public class JsonResponseOutputterController_SADH {
	private static Logger logger = LoggerFactory.getLogger(JsonResponseOutputterController_SADH.class.getName());
	/**
	 * FreeForm Source:
	 * 	 File: 		SADH
	 ...
	 * 
	 * @return
	 * @Example SELECT *: http://gw.systema.no:8080/syjservicestn/syjsSADHF.do?user=OSCAR
	 * @Example SELECT specific: http://gw.systema.no:8080/syjservicestn/syjsSADH-SYS.do?user=OSCAR&siavd=whatever...
	 * 
	 */
	@RequestMapping(value="syjsSADH-SYS.do", method={RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
		...
		try{
			...
			if(userName!=null && !"".equals(userName)){
	            ...
	            if( this.isDoFind(dao) ){
					...
					list = this.sadhDaoServices.find(dao, dbErrorStackTrace);
				}else{
					logger.warn("inside: getList (all)");
					logger.warn("getList (all)");
					list = this.sadhDaoServices.getList(dbErrorStackTrace);
				}
	 ...
	 * @return
	 */
	private boolean isDoFind(SadhDao dao){
		boolean retval = false;
		
		if(StringUtils.isNotEmpty(dao.getSiavd()) || StringUtils.isNotEmpty(dao.getSitdn())
				|| StringUtils.isNotEmpty(dao.getSist()) || StringUtils.isNotEmpty(dao.getSidt()) ){ 
