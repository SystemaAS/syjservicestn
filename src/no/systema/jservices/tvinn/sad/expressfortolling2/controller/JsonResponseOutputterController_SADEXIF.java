...
import no.systema.jservices.tvinn.sad.expressfortolling2.controller.rules.SADEXIF_U;
...
import no.systema.jservices.tvinn.sad.expressfortolling2.model.dao.services.SadexifDaoServices;
...
@Controller
public class JsonResponseOutputterController_SADEXIF {
	private static Logger logger = LoggerFactory.getLogger(JsonResponseOutputterController_SADEXIF.class.getName());
	/**
	 * FreeForm Source:
	 * 	 File: 		SADEXIF - GOODS ITEM
	 * 	 Member: 	SAD Ekpressfortolling - SELECT LIST or SELECT SPECIFIC
	 *  
	 * 
	 * @return
	 * @Example SELECT *: http://gw.systema.no:8080/syjservicestn/syjsSADEXIF.do?user=OSCAR
	 * @Example SELECT specific: http://gw.systema.no:8080/syjservicestn/syjsSADEXIF.do?user=OSCAR&eiavd=whatever...
	 * 
	 */
	@RequestMapping(value="syjsSADEXIF.do", method={RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public String syjsRList( HttpSession session, HttpServletRequest request) {
		...
		try{
			logger.warn("Inside syjsSADEXIF");
			...
			if(userName!=null && !"".equals(userName)){
	            ...
	            if( this.isDoFind(dao) ){
					logger.warn("find");
					list = this.sadexifDaoServices.find(dao, dbErrorStackTrace);
					
				}else{
					logger.warn("getList (all)");
					list = this.sadexifDaoServices.getList(dbErrorStackTrace);
				}
	 ...
	 * @return
	 */
	private boolean isDoFind(SadexifDao dao){
		boolean retval = false;
		if(dao.getEiavd()>0 || dao.getEipro()>0 || dao.getEitdn()>0 || StringUtils.isNotEmpty(dao.getEist()) || dao.getEili()>0 ){
			retval = true;
		}
	...
	@RequestMapping(value="syjsSADEXIF_U.do", method={RequestMethod.GET, RequestMethod.POST})
	@ResponseBody
	public String syjsR_U( HttpSession session, HttpServletRequest request) {
		...
		try{
			logger.info("Inside syjsSADEXIF_U.do");
			//TEST-->logger.info("Servlet root:" + AppConstants.VERSION_SYJSERVICES);
			String user = request.getParameter("user");
            ...
            logger.warn("tdn:" + dao.getEitdn().toString());
            logger.warn("line:" + dao.getEili().toString());
            logger.warn("user:" + user);
            logger.warn("mode:" + mode);
            
            //rules
            SADEXIF_U rulerLord = new SADEXIF_U();
			//Start processing now
			if(userName!=null && !"".equals(userName)){
				int dmlRetval = 0;
				if("D".equals(mode)){
				  ...
				  if(rulerLord.isValidInputForUpdate(dao, userName, mode)){
						...
						List<SadexifDao> list = new ArrayList<SadexifDao>();
	...
	//----------------
	@Autowired
	private SadexifDaoServices sadexifDaoServices;
